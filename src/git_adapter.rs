pub mod git_adapter {
                    ...
                    Some(ObjectType::Tree) => {
                        ...
                        match ledger_blob_entry.kind() {
                            Some(ObjectType::Blob) => {
                                match repo.find_blob(ledger_blob_entry.id()) {
                                    Ok(blob) => match str::from_utf8(blob.content()) {
                                        Ok(text) => match toml::from_str::<structs::Ledger>(text) {
                                            Ok(ledger) => {
                                                let ledger_path = ledgers_path.join(ledger_name);
    ...
    /// Parse `group.toml` located at the repository root into a `structs::Group`.
    ///
    /// Looks up `group.toml` in the root tree of the commit resolved by refs/heads/main or HEAD.
    pub fn get_group(repo: &Repository) -> Result<structs::Group, &'static str> {
        let root_tree = get_root_tree(repo)?;

        // Try to find group.toml at the repository root
        let entry = match root_tree.get_path(Path::new("group.toml")) {
            Ok(e) => e,
            Err(_) => return Err("group.toml not found in repository root"),
        };
        ...
        match entry.kind() {
            Some(ObjectType::Blob) => {
                let blob = repo
                    .find_blob(entry.id())
                    .map_err(|_| "failed to read group.toml blob")?;
                let text =
                    str::from_utf8(blob.content()).map_err(|_| "group.toml is not valid utf8")?;
                toml::from_str::<structs::Group>(text).map_err(|_| "failed to parse group.toml")
            }
            Some(_) => Err("group.toml is not a blob"),
            None => Err("group.toml entry has no object type"),
        }
    }
}
